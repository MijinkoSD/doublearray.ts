...
(() => {
    ...
    BASE_SIGNED = true, CHECK_SIGNED = true, BASE_BYTES = 4, CHECK_BYTES = 4, MEMORY_EXPAND_RATIO = 2;
    let bc_member;
    let keys_member;
    const newBC = function (initial_size) {
        ...
        const initBase = function (_base, start, end) {
        ...
        const initCheck = function (_check, start, end) {
            for (let i = start; i < end; i++) {
     ...
     */
    const DoubleArrayBuilder = (initial_size) => {
        bc_member = newBC(initial_size); // BASE and CHECK
        keys_member = [];
        const append = (key, record) => {
            keys_member.push({ k: key, v: record });
        };
        const build = (keys, sorted) => {
            if (keys == null) {
                keys = keys_member;
            }
            if (keys == null) {
        ...
        const _build = (parent_index, position, start, length) => {
        ...
        const getChildrenInfo = (position, start, length) => {
        ...
        const setBC = (parent_id, children_info, _base) => {
            var bc = bc_member;
        ...
        const findAllocatableBase = (children_info) => {
            var bc = bc_member;
